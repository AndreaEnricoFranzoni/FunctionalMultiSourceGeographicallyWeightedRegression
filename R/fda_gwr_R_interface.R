...
#' @param knots_y_points **`numeric vector`** of double with the abscissa points with respect which the basis expansions of the response and response reconstruction weights are performed (all elements contained in the functional data domain). 
...
#' @param knots_stationary_cov **`numeric vector`** of double with the abscissa points with respect which the basis expansions of the stationary covariates are performed (all elements contained in the functional data domain). 
...
#' @param n_basis_stationary_cov **`integer vector`** of positive integers: element i-th is the number of basis for the basis expansion of the i-th stationary covariate. It must match number of rows of the i-th element of coeff_stationary_cov. Default explained below (can be **`NULL`**).
...
#' @param knots_beta_stationary_cov **`numeric vector`** of double with the abscissa points with respect which the basis expansions of the stationary covariates functional regression coefficients are performed (all elements contained in the functional data domain).
...
#' @param knots_events_cov **`numeric vector`** of double with the abscissa points with respect which the basis expansions of the events-dependent covariates are performed (all elements contained in the functional data domain).                        
...
#' @param n_basis_events_cov **`integer vector`** of positive integers: element i-th is the number of basis for the basis expansion of the i-th events-dependent covariate. It must match number of rows of the i-th element of coeff_events_cov. Default explained below (can be **`NULL`**).
...
#' @param knots_beta_events_cov **`numeric vector`** of double with the abscissa points with respect which the basis expansions of the events-dependent covariates functional regression coefficients are performed (all elements contained in the functional data domain). 
...
#' @param knots_stations_cov **`numeric vector`** of double with the abscissa points with respect which the basis expansions of the stations-dependent covariates are performed (all elements contained in the functional data domain). 
...
#' @param n_basis_stations_cov **`integer vector`** of positive integers: element i-th is the number of basis for the basis expansion of the i-th stations-dependent covariate. It must match number of rows of the i-th element of coeff_stations_cov. Default explained below (can be **`NULL`**).
...
#' @param knots_beta_stations_cov **`numeric vector`** of double with the abscissa points with respect which the basis expansions of the stations-dependent covariates functional regression coefficients are performed (all elements contained in the functional data domain). 
...
#' @name FMSGWR_SEC
#' @description
#' Fitting a Functional Multi-Source Geographically Weighted Regression SEC model. The covariates are functional objects, divided into
#' three categories: stationary covariates (C), constant over geographical space, event-dependent covariates (E), that vary depending on the spatial coordinates of the event, 
#' station-dependent covariates (S), that vary depending on the spatial coordinates of the stations that measure the event. Regression coefficients are estimated 
#' in the following order: C, E, S. The functional response is already reconstructed according to the method proposed by Bortolotti et Al. (2024)
#' @param y_points **`numeric matrix`** of double containing the raw response: each row represents a specific abscissa for which the response evaluation is available, each column a statistical unit. Response is a already reconstructed.
#' @param t_points **`numeric vector`** of double with the abscissa points with respect of the raw evaluations of y_points are available (length of t_points is equal to the number of rows of y_points).
#' @param left_extreme_domain **`double`** indicating the left extreme of the functional data domain (not necessarily the smaller element in t_points).
#' @param right_extreme_domain double indicating the right extreme of the functional data domain (not necessarily the biggest element in t_points).
...
#'                               \itemize{
#'                               \item 'c_tilde_hat': **`numeric vector`** of double with the basis expansion coefficients of the response minus the stationary component of the phenomenon (if in_cascade_estimation is true, contains only 0s).
#'                               \item 'A__': **`list of numeric matrices`**, containing the operator A_s for each statistical unit (if in_cascade_estimation is true, each matrix contains only 0s).
#'                               \item 'B__for_K': **`list of numeric matrices`**, containing the operator B_s used for the K_s_e(t) computation, for each statistical unit (if in_cascade_estimation is true, each matrix contains only 0s).
#'                               }
#'                         \item 'inputs_info': **`list`** containing information about the data used to fit the model:
#'                               \itemize{
#'                               \item 'Response': **`list`**:
...
#' @description
#' Function to perform predictions on new statistical units using a fitted Functional Multi-Source Geographically Weighted Regression SEC model. Non-stationary betas have to be recomputed in the new locations.
#' @param coeff_stationary_cov_to_pred **`list of numeric matrices`** of double: element i-th containing the coefficients for the basis expansion of the i-th stationary covariate to be predicted: each row represents a specific basis (by default: B-spline) of the basis system used, each column a statistical unit to be predicted.
#' @param coeff_events_cov_to_pred **`list of numeric matrices`** of double: element i-th containing the coefficients for the basis expansion of the i-th event-dependent covariate to be predicted: each row represents a specific basis (by default: B-spline) of the basis system used, each column a statistical unit to be predicted.
#' @param coordinates_events_to_pred **`numeric matrix`**  of double containing the UTM coordinates of the event of new statistical units: each row represents a statistical unit to be predicted, each column a coordinate (2 columns).
#' @param coeff_stations_cov_to_pred **`list of numeric matrices`** of double: element i-th containing the coefficients for the basis expansion of the i-th station-dependent covariate to be predicted: each row represents a specific basis (by default: B-spline) of the basis system used, each column a new statistical unit.
...
#' @name FMGWR
#' @description
#' Fitting a Functional Mixed Geographically Weighted Regression model. The covariates are functional objects, divided into two categories:
#' stationary covariates (C), constant over geographical space, and non-stationary covariates (NC), that vary depending on spatial coordinates. Regression coefficients are estimated  
#' in the following order: C, NC. The functional response is already reconstructed according to the method proposed by Bortolotti et Al. (2024) 
#' @param y_points **`numeric matrix`** of double containing the raw response: each row represents a specific abscissa for which the response evaluation is available, each column a statistical unit. Response is a already reconstructed.
#' @param t_points **`numeric vector`** of double with the abscissa points with respect of the raw evaluations of y_points are available (length of t_points is equal to the number of rows of y_points).
#' @param left_extreme_domain **`double`** indicating the left extreme of the functional data domain (not necessarily the smaller element in t_points).
#' @param right_extreme_domain double indicating the right extreme of the functional data domain (not necessarily the biggest element in t_points).
...
#' @param degrees_basis_beta_stationary_cov **`integer vector`**  of non-negative integers: element i-th is the degree of the basis used for the basis expansion of the i-th stationary covariate functional regression coefficients. Default explained below (can be **`NULL`**).
#' @param n_basis_beta_stationary_cov **`integer vector`** of positive integers: element i-th is the number of basis for the basis expansion of the i-th stationary covariate functional regression coefficients. Default explained below (can be **`NULL`**).
#' @param coeff_non_stationary_cov **`list of numeric matrices`** of double: element i-th containing the coefficients for the basis expansion of the i-th non-stationary covariate: each row represents a specific basis (by default: B-spline) of the basis system used, each column a statistical unit.
...
#' @param knots_non_stationary_cov **`numeric vector`** of double with the abscissa points with respect which the basis expansions of the non-stationary covariates are performed (all elements contained in the functional data domain).                        
#' @param degrees_basis_non_stationary_cov **`integer vector`** of non-negative integers: element i-th is the degree of the basis used for the basis expansion of the i-th non-stationary covariate. Default explained below (can be **`NULL`**).
#' @param n_basis_non_stationary_cov **`integer vector`** of positive integers: element i-th is the number of basis for the basis expansion of the i-th non-stationary covariate. It must match number of rows of the i-th element of coeff_non_stationary_cov. Default explained below (can be **`NULL`**).
#' @param penalization_non_stationary_cov **`numeric vector`** of non-negative double: element i-th is the penalization used for the i-th non-stationary covariate.
#' @param coordinates_non_stationary **`numeric matrix`** of double containing the UTM coordinates of the non-stationary site of each statistical unit: each row represents a statistical unit, each column a coordinate (2 columns).
...
#' @param knots_beta_non_stationary_cov **`numeric vector`** of double with the abscissa points with respect which the basis expansions of the non-stationary covariates functional regression coefficients are performed (all elements contained in the functional data domain). 
#' @param degrees_basis_beta_non_stationary_cov **`integer vector`** of non-negative integers: element i-th is the degree of the basis used for the basis expansion of the i-th non-stationary covariate functional regression coefficient. Default explained below (can be **`NULL`**). 
#' @param n_basis_beta_non_stationary_cov **`integer vector`** of positive integers: element i-th is the number of basis for the basis expansion of the i-th non-stationary covariate functional regression coefficient. Default explained below (can be **`NULL`**).
#' @param in_cascade_estimation **`bool`**: if false, an exact algorithm taking account for the interaction within non-stationary covariates is used to fit the model. Otherwise, the model is fitted in cascade. The first option is more precise, but way more computationally intensive.
#' @param n_knots_smoothing **`positive integer`**: number of knots used to perform the smoothing on the response obtained leaving out all the non-stationary components (default: **`100`**).
#' @param n_intervals_quadrature **`positive integer`**: number of intervals used while performing integration via midpoint (rectangles) quadrature rule (default: **`100`**).
#' @param num_threads **`positive integer`**: number of threads to be used in OMP parallel directives. Default: **`NULL`**, that is equivalent to the maximum number of cores available in the machine.
...
#' @param basis_type_rec_weights_y_points **`string`** containing the type of basis used for the weights to reconstruct the functional response basis expansion. Possible values: **`"bsplines"`**, **`"constant"`**. Defalut: **`"bsplines"`**.
#' @param basis_types_beta_stationary_cov **`vector of strings`**, element i-th containing the type of basis used for the i-th stationary covariate functional regression coefficients basis expansion. Possible values: **`"bsplines"`**, **`"constant"`**. Defalut: **`"bsplines"`**.
#' @param basis_types_beta_non_stationary_cov **`vector of strings`**, element i-th containing the type of basis used for the i-th non-stationary covariate functional regression coefficients basis expansion. Possible values: **`"bsplines"`**, **`"constant"`**. Defalut: **`"bsplines"`**.
#' @return **`list`** containing:
#'                   \itemize{
...
#' @name FGWR
#' @description
#' Fitting a Functional Geographically Weighted Regression model. The covariates are functional objects, non-stationary covariates (NC), that vary depending on spatial coordinates.
...
#' @return **`list`** containing:
#'                   \itemize{
#'                   \item 'FGWR': **`string`**: the type of fwr used (**`"FWR"`**);
#'                   \item 'Bc': **`list`** containing, for each stationary covariate regression coefficient (each element is named with the element names in the list coeff_stationary_cov (default, if not given: **`"CovC*"`**)):
#'                         \itemize{
#'                         \item 'basis_coeff': **`numeric vector`** of double, dimension Lc_jx1,containing the coefficients of the basis expansion of the beta.
#'                         \item 'basis_type': **`string`** containing the basis type over which the beta basis expansion is performed. Possible values: **`"bsplines"`**, **`"constant"`**. (Respective element of basis_types_beta_stationary_cov).
...
#' @return **`list`** containing:
#'         \itemize{
#'         \item 'FGWR_predictor': **`string`**: model used to predict (**`"predictor_FWR"`**).
#'         \item 'prediction': **`list`** containing:
#'               \itemize{
#'               \item 'evaluation': **`list`** containing the evaluation of the prediction:
#'                     \itemize{
